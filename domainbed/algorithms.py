...
class CoOp(DPCLIP):
    """
    CoOp generate prompt with specific variables.
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(CoOp, self).__init__(input_shape, num_classes, num_domains, hparams)
        
        #  initial prompt.
        ctx_vectors = torch.empty(hparams['num_domain_tokens'], self.EMBEDDING_DIM, dtype=self.clip_model.dtype)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        image_features = self.clip_model.encode_image(all_x)
        # generate prompt with self.ctx.
        text_features = self._get_text_features(self.ctx, coop=True)
    ...
    def predict(self, x):
        image_feature = self.clip_model.encode_image(x)
        # generate prompt with self.ctx.
        text_feature = self._get_text_features(self.ctx, coop=True)
        image_feature = image_feature / image_feature.norm(dim=-1, keepdim=True)
        text_feature = text_feature / text_feature.norm(dim=-1, keepdim=True)
        return self.clip_model.logit_scale.exp() * image_feature @ text_feature.t()
        

class UDGCLIP(DPCLIP):
    """
    UDG.
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(UDGCLIP, self).__init__(input_shape, num_classes, num_domains, hparams)
        dim_domain_feature = self.EMBEDDING_DIM * hparams['num_domain_tokens']
        
        self.domain_classification_network = networks.Classifier(
                                        dim_domain_feature,
                                        num_domains,
                                        self.hparams
        )
        self.optimizer = torch.optim.SGD(params=[
            {"params": self.network.parameters(), "lr": self.hparams["lr"]},
            {"params": self.domain_classification_network.parameters(), "lr": self.hparams["lr"]},]
        ) 
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        loss = self.domain_loss(domain_features)
    ...
    def domain_loss(self, domain_features):
        num_batch = len(domain_features[0])
        all_domain_feature = torch.cat(domain_features)
        
        prediction = self.domain_classification_network(all_domain_feature)
        y = torch.tensor([0] * num_batch + [1] * num_batch + [2] * num_batch, device=self.device).long()
        loss = F.cross_entropy(prediction, y)
        # print('domain_loss: ', loss.item())
        return loss
...
class AbstractDANN(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        grad_loss = (input_grad**2).sum(dim=1).mean(dim=0)
        disc_loss += self.hparams['grad_penalty'] * grad_loss
...
class DANN(AbstractDANN):
    """Unconditional DANN"""
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DANN, self).__init__(input_shape, num_classes, num_domains, hparams, conditional=False, class_balance=False)


class DANNCLIP(AbstractDANN):
    """Unconditional DANN"""
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DANNCLIP, self).__init__(
                            input_shape, 
                            num_classes, 
                            num_domains,
                            hparams, 
